...
class ColorAnomalyAndMotionDetectionControlWidget(TranslationMixin, QWidget):
    ...
    def _create_motion_tab(self) -> QWidget:
        ...
        # Enable Motion
        self.enable_motion = QCheckBox(self.tr("Enable Motion Detection"))
        self.enable_motion.setChecked(False)
        self.enable_motion.setToolTip(self.tr(
            "Turn ON to highlight moving objects in the scene.\n"
        ...
        # Algorithm Selection
        algo_group = QGroupBox(self.tr("Algorithm"))
        algo_layout = QGridLayout(algo_group)
        algo_layout.setColumnMinimumWidth(0, 160)
        algo_layout.setColumnStretch(1, 1)

        algo_layout.addWidget(QLabel(self.tr("Type:")), 0, 0)
        self.motion_algorithm = QComboBox()
        self.motion_algorithm.addItem(self.tr("FRAME_DIFF"), MotionAlgorithm.FRAME_DIFF)
        self.motion_algorithm.addItem(self.tr("MOG2"), MotionAlgorithm.MOG2)
        self.motion_algorithm.addItem(self.tr("KNN"), MotionAlgorithm.KNN)
        self.motion_algorithm.setCurrentIndex(1)
        self.motion_algorithm.setToolTip(self.tr(
            "Motion detection algorithm (advanced setting):\n\n"
        ...
        param_group = QGroupBox(self.tr("Detection Parameters"))
        param_layout = QGridLayout(param_group)
        param_layout.setColumnMinimumWidth(0, 160)
        param_layout.setColumnStretch(1, 1)

        row = 0
        param_layout.addWidget(QLabel(self.tr("Motion Threshold:")), row, 0)
        self.motion_threshold = QSpinBox()
        self.motion_threshold.setRange(1, 255)
        self.motion_threshold.setValue(10)
        self.motion_threshold.setToolTip(self.tr(
            "Minimum pixel intensity change to consider as motion (1-255).\n"
        ...
        row += 1
        param_layout.addWidget(QLabel(self.tr("Blur Kernel (odd):")), row, 0)
        ...
        self.blur_kernel_size.setToolTip(self.tr(
            "Gaussian blur kernel size (must be odd: 1, 3, 5, 7, etc.).\n"
        ...
        row += 1
        param_layout.addWidget(QLabel(self.tr("Morphology Kernel:")), row, 0)
        ...
        self.morphology_kernel_size.setToolTip(self.tr(
            "Morphological operation kernel size (odd numbers: 1, 3, 5, etc.).\n"
        ...
        persist_layout.addWidget(QLabel(self.tr("Threshold (N of M):")), 1, 0)
        self.persistence_threshold = QSpinBox()
        self.persistence_threshold.setRange(1, 30)
        self.persistence_threshold.setValue(2)
        self.persistence_threshold.setToolTip(self.tr(
            "Number of frames within window where motion must appear (N of M).\n"
        ...
        bg_group = QGroupBox(self.tr("Background Subtraction (MOG2/KNN)"))
        bg_layout = QGridLayout(bg_group)
        bg_layout.setColumnMinimumWidth(0, 160)
        bg_layout.setColumnStretch(1, 1)

        bg_layout.addWidget(QLabel(self.tr("History Frames:")), 0, 0)
        self.bg_history = QSpinBox()
        self.bg_history.setRange(10, 500)
        self.bg_history.setValue(50)
        self.bg_history.setToolTip(self.tr(
            "Number of frames to learn background model (10-500).\n"
        ...
        bg_layout.addWidget(QLabel(self.tr("Variance Threshold:")), 1, 0)
        self.bg_var_threshold = QDoubleSpinBox()
        self.bg_var_threshold.setRange(1.0, 100.0)
        self.bg_var_threshold.setValue(10.0)
        self.bg_var_threshold.setToolTip(self.tr(
            "Variance threshold for background/foreground classification (1.0-100.0).\n"
            "Only applies to MOG2 and KNN algorithms.\n"
        ...
        self.bg_detect_shadows = QCheckBox(self.tr("Detect Shadows (slower)"))
        self.bg_detect_shadows.setToolTip(self.tr(
            "Enables shadow detection in MOG2 background subtractor.\n"
        ...
        size_group = QGroupBox(self.tr("Object Size Filter"))
        size_layout = QGridLayout(size_group)
        size_layout.setColumnMinimumWidth(0, 160)
        size_layout.setColumnStretch(1, 1)

        size_layout.addWidget(QLabel(self.tr("Min Object Area (px):")), 0, 0)
        self.min_detection_area = QSpinBox()
        self.min_detection_area.setRange(1, 100000)
        self.min_detection_area.setValue(5)
        self.min_detection_area.setToolTip(self.tr(
            "Minimum detection area in pixels (1-100000).\n"
        ...
        size_layout.addWidget(QLabel(self.tr("Max Object Area (px):")), 1, 0)
        self.max_detection_area = QSpinBox()
        self.max_detection_area.setRange(10, 1000000)
        self.max_detection_area.setValue(1000)
        self.max_detection_area.setToolTip(self.tr(
            "Maximum detection area in pixels (10-1000000).\n"
        ...
        # Camera Movement
        cam_group = QGroupBox(self.tr("Camera Movement Detection"))
        cam_layout = QVBoxLayout(cam_group)

        self.pause_on_camera_movement = QCheckBox(self.tr("Pause on Camera Movement"))
        self.pause_on_camera_movement.setChecked(True)
        self.pause_on_camera_movement.setToolTip(self.tr(
            "Automatically pauses motion detection when camera is moving/panning.\n"
        ...
        cam_thresh_layout.addWidget(QLabel(self.tr("Threshold:")))
        ...
        self.camera_movement_threshold.setToolTip(self.tr(
            "Percentage of frame with motion to consider as camera movement (1-100%).\n"
        ...
        self.show_advanced_motion = QCheckBox(self.tr("Show Advanced Motion Settings"))
        self.show_advanced_motion.setChecked(False)
        self.show_advanced_motion.setToolTip(self.tr(
            "Advanced users can expand this to adjust the motion algorithm\n"
    ...
    def _create_color_tab(self) -> QWidget:
        ...
        # Enable Color
        self.enable_color_quantization = QCheckBox(self.tr("Enable Color-Based Anomaly Detection"))
        self.enable_color_quantization.setChecked(True)
        self.enable_color_quantization.setToolTip(self.tr(
            "Detects pixels whose colors are statistically rare in the frame.\n"
        ...
        quant_group = QGroupBox(self.tr("Color Rarity Settings"))
        quant_layout = QGridLayout(quant_group)
        quant_layout.setColumnMinimumWidth(0, 160)
        quant_layout.setColumnStretch(1, 1)

        quant_layout.addWidget(QLabel(self.tr("Color Resolution (bins):")), 0, 0)
        ...
        self.color_quantization_bits.setToolTip(self.tr(
            "Controls how finely colors are grouped into histogram bins (3-8 bits).\n"
        ...
        quant_layout.addWidget(QLabel(self.tr("Rarity Threshold (% of colors):")), 1, 0)
        ...
        self.color_rarity_percentile.setToolTip(self.tr(
            "Sensitivity threshold for how rare a color must be to be flagged (0-100%).\n"
        ...
        quant_layout.addWidget(QLabel(self.tr("Min Object Area (px):")), 2, 0)
        ...
        self.color_min_detection_area.setToolTip(self.tr(
            "Minimum area in pixels for a color anomaly to be treated as an object of interest.\n"
        ...
        quant_layout.addWidget(QLabel(self.tr("Max Object Area (px):")), 3, 0)
        ...
        self.color_max_detection_area.setToolTip(self.tr(
            "Maximum area in pixels for a color anomaly to be treated as an object of interest.\n"
        ...
        quant_layout.addWidget(QLabel(self.tr("Blob Detection Method:")), 4, 0)
        ...
        self.contour_method.setToolTip(self.tr(
            "Method for extracting blob regions from the detection mask:\n\n"
        ...
        colorspace_group = QGroupBox(self.tr("Color Space (Lighting Invariance)"))
        ...
        colorspace_layout.addWidget(QLabel(self.tr("Color Space:")), 0, 0)
        self.color_space = QComboBox()
        self.color_space.addItem(self.tr("RGB"), ColorSpace.BGR)
        self.color_space.addItem(self.tr("HSV"), ColorSpace.HSV)
        self.color_space.addItem(self.tr("LAB"), ColorSpace.LAB)
        self.color_space.setCurrentIndex(0)
        self.color_space.setToolTip(self.tr(
        ...
        self.hsv_sat_label_widget = QLabel(self.tr("HSV Min Saturation:"))
        ...
        self.hsv_min_saturation.setToolTip(self.tr(
            "Minimum saturation for HSV mode (0-255).\n"
        ...
        self.lab_chroma_label_widget = QLabel(self.tr("LAB Min Chroma:"))
        ...
        self.lab_min_chroma.setToolTip(self.tr(
            "Minimum chroma (color intensity) for LAB mode (0-128).\n"
        ...
        # Hue Expansion
        hue_group = QGroupBox(self.tr("Color Match Expansion"))
        hue_layout = QVBoxLayout(hue_group)

        self.enable_hue_expansion = QCheckBox(self.tr("Allow Similar Colors (Hue Expansion)"))
        self.enable_hue_expansion.setChecked(False)  # Default OFF
        self.enable_hue_expansion.setToolTip(self.tr(
            "Lets the detector treat similar colors as the same object.\n"
        ...
        hue_range_layout.addWidget(QLabel(self.tr("Color Match Range:")))
        self.hue_expansion_range = QSlider(Qt.Horizontal)
        self.hue_expansion_range.setRange(0, 30)
        self.hue_expansion_range.setValue(5)
        self.hue_expansion_range.setToolTip(self.tr(
            "How wide to stretch the color match around each detected color.\n"
        ...
        exclusion_group = QGroupBox(self.tr("Color Exclusion"))
        exclusion_layout = QVBoxLayout(exclusion_group)

        self.enable_color_exclusion = QCheckBox(self.tr("Enable Color Exclusion"))
        self.enable_color_exclusion.setChecked(False)
        self.enable_color_exclusion.setToolTip(self.tr(
            "Exclude specific background colors from color anomaly detection.\n"
        ...
        exclusion_layout.addWidget(QLabel(self.tr("Click on color wheel to exclude colors (20° steps, 0-360°):")))

        # Create color wheel widget
        self.color_wheel = ColorWheelWidget(size=300)
        self.color_wheel.setToolTip(self.tr(
            "Click on any color segment to toggle exclusion on/off.\n"
    ...
    def _create_fusion_tab(self) -> QWidget:
        ...
        # Fusion
        fusion_group = QGroupBox(self.tr("Detection Fusion"))
        fusion_layout = QVBoxLayout(fusion_group)

        self.enable_fusion = QCheckBox(self.tr("Enable Fusion (when both motion and color enabled)"))
        self.enable_fusion.setChecked(False)  # Default OFF
        self.enable_fusion.setToolTip(self.tr(
            "Combines motion and color detections when both are enabled.\n"
        ...
        mode_layout.addWidget(QLabel(self.tr("Fusion Mode:")))
        self.fusion_mode = QComboBox()
        self.fusion_mode.addItem(self.tr("UNION"), FusionMode.UNION)
        self.fusion_mode.addItem(self.tr("INTERSECTION"), FusionMode.INTERSECTION)
        self.fusion_mode.addItem(self.tr("COLOR_PRIORITY"), FusionMode.COLOR_PRIORITY)
        self.fusion_mode.addItem(self.tr("MOTION_PRIORITY"), FusionMode.MOTION_PRIORITY)
        self.fusion_mode.setCurrentIndex(0)
        self.fusion_mode.setToolTip(self.tr(
            "How to combine motion and color detections:\n\n"
