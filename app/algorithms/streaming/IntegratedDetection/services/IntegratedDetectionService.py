"""
IntegratedDetectionService.py - Integrated real-time anomaly detection
...
This service is part of the IntegratedDetection algorithm module.
"""
...
class IntegratedDetectionConfig:
    ...
    color_quantization_bits: int = 4  # Bits per channel (4 = 16^3 = 4,096 bins, matches UI default)
    color_rarity_percentile: float = 30.0  # Mark colors below this percentile as rare (matches UI default)
    color_min_detection_area: int = 15  # Minimum area for color anomalies (matches UI default)
    ...
    enable_hue_expansion: bool = False  # Expand detections to neighboring pixels with similar hue (matches UI default: OFF)
...
class IntegratedDetectionService(QObject):
    """
    ...
    def _color_quantization_detect(self, frame_bgr: np.ndarray, config: IntegratedDetectionConfig, max_detections: int = 0) -> List[Detection]:
        ...
        2. Quantize RGB to N bits/channel (e.g., 4 bits = 16^3 = 4,096 bins)
        ...
        # === DEBUG OUTPUT ===
        print("\n=== COLOR ANOMALY DETECTION DEBUG ===")
        print(f"Frame Resolution: {w_orig}x{h_orig} (original) -> {w}x{h} (downsampled 2x)")
        print(f"Quantization: {bits} bits/channel = {bins_per_channel}^3 = {max_bins} total bins")
        print(f"Total pixels analyzed: {total_pixels}")
        print(f"Unique colors found: {len(nonzero_counts)} out of {max_bins} possible")
        print(f"Rarity percentile setting: {config.color_rarity_percentile}%")
        print(f"Percentile threshold (P{config.color_rarity_percentile}): {percentile_threshold:.1f} pixels")
        print(f"Absolute max (5% cap): {absolute_max:.1f} pixels")
        print(f"Final threshold used: {threshold:.1f} pixels (min of above two)")
        print(f"Min detection area: {config.color_min_detection_area}px")
        print(f"Max detection area: {config.max_detection_area}px (using shared max_detection_area)")
        print(f"Morphology: ALWAYS applied (kernel size: {config.morphology_kernel_size})")
        print(f"Hue expansion enabled: {config.enable_hue_expansion} (range: {config.hue_expansion_range})")

        # Create mask of rare bins (matching original - no ceil)
        rare_bins = (histogram > 0) & (histogram < threshold)
        rare_bins[0] = False  # Don't mark bin 0 as rare (often black/background)

        num_rare_bins = np.sum(rare_bins)
        print(f"Rare color bins found: {num_rare_bins} ({100.0 * num_rare_bins / len(nonzero_counts):.1f}% of unique colors)")
        ...
        # Original always applies morphology (no conditional check)
        morph_kernel = self._get_morph_kernel(config.morphology_kernel_size)
        ...
        for contour in contours:
            ...
            # Filter by area (original uses max_detection_area, not color_max_detection_area)
            ...
            # Scale centroid to downsampled coordinates (matching original)
            cy_down = cy // 2
            cx_down = cx // 2
            # Bounds check
        ...
        print(f"Contours filtered by area: {filtered_by_area}")
        ...
        if config.enable_hue_expansion and len(detections) > 0 and config.hue_expansion_range > 0:
            ...
            for detection in detections:
                ...
                # Re-check area filter after expansion (original uses max_detection_area)
            ...
            print(f"Detections after hue expansion: {len(detections)}")

        print(f"FINAL: {len(detections)} color anomaly detections returned")
        print("=====================================\n")
    ...
    def _apply_color_exclusion_filter(self, detections: List[Detection], frame_bgr: np.ndarray,
        ...
        for detection in detections:
            ...
            # Prefer hue derived from detected dominant color (more accurate)
            ...
            if hue_value is None:
                ...
                # Clamp bounds to frame to ensure exclusion still works near edges
                x = max(0, min(x, w_frame - 1))
                y = max(0, min(y, h_frame - 1))
    ...
    def process_frame(self, frame: np.ndarray, timestamp: float) -> Tuple[np.ndarray, List[Detection], StageTimings]:
        ...
        # Show camera movement indicator at top of screen (only matters when motion detection is enabled)
        ...
        # Provide resolution metadata for downstream consumers (thumbnails, etc.)
...
RealtimeIntegratedDetector = IntegratedDetectionService
