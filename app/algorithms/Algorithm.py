...
class AlgorithmService:
    ...
    def circleAreasOfInterest(self, img, contours):
        ...
        :return numpy.ndarray, List, int: numpy.ndarray representing the output image, a list of areas of interest, and a count of the original areas of
        interest before they were combined to eliminate overlapping circles
        """

        found = False
        # 3 step process.  Step 1, find all the areas >= the minimum size and mark
        # them on a temporary mask.  Step 2, loop through combining any circles
        # that overlap. Step 3, draw the final list of circles on a copy of the
        # original image.
