"""
RealtimeMotionDetectionService.py - Advanced motion detection for static and moving cameras
...
class RealtimeMotionDetector(QObject):
    """
    ...
    def _estimate_camera_motion(self, prev_gray: np.ndarray, curr_gray: np.ndarray) -> Optional[CameraMotion]:
        ...
        try:
            ...
            if prev_gray.shape != curr_gray.shape:
                self.logger.warning(f"Camera motion estimation frame size mismatch. Skipping.")
                return None
    ...
    def _detect_frame_diff(self, frame: np.ndarray, gray: np.ndarray, motion_threshold: int, morphology_size: int, min_area: int, max_area: int) -> List[MotionDetection]:
    ...
    def _detect_optical_flow(self, gray: np.ndarray, motion_threshold: int, morphology_size: int, min_area: int, max_area: int) -> List[MotionDetection]:
        ...
        # Check if frame sizes match (in case of resolution change)
        if self._prev_gray.shape != gray.shape:
            self.logger.warning(f"Optical flow frame size mismatch: prev={self._prev_gray.shape}, curr={gray.shape}. Skipping frame.")
            self._prev_gray = gray.copy()
            return detections
        ...
        try:
            with self._config_lock:
                ...
                if resolution_changed:
                    ...
                    self._init_background_subtractors()
                    self.logger.info(f"Cleared frame buffers due to resolution change")
