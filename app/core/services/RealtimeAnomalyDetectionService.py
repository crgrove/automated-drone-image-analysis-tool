...
class RealtimeAnomalyDetector(QObject):
    ...
    def _detect_cpu(self, frame: np.ndarray, timestamp: float, 
                   config: AnomalyConfig) -> List[AnomalyDetection]:
    ...
    def _detect_gpu(self, frame: np.ndarray, timestamp: float,
                   config: AnomalyConfig) -> List[AnomalyDetection]:
    ...
    def _process_contours(self, contours: List, anomaly_map: np.ndarray, 
                         timestamp: float, config: AnomalyConfig) -> List[AnomalyDetection]:
    ...
    def create_annotated_frame(self, frame: np.ndarray, detections: List[AnomalyDetection], 
                              anomaly_map: Optional[np.ndarray] = None) -> np.ndarray:
        """
