...
class FrameQueue:
    ...
    def put(self, frame: np.ndarray, timestamp: float, frame_number: int):
        """
        Add a frame to the queue, dropping the old frame if present.
        ...
        with self._lock:
            if self._frame is not None:
                # We're dropping an unprocessed frame
                self._dropped_frames += 1

            self._frame = frame.copy()  # Copy to avoid race conditions
    ...
    def get(self, timeout: Optional[float] = None) -> Optional[Tuple[np.ndarray, float, int]]:
        ...
        Args:
            timeout: Maximum time to wait for a frame (None = wait forever)

        Returns:
            Tuple of (frame, timestamp, frame_number) or None if timeout
        """
...
class ThreadedCaptureWorker:
    """
    Background thread worker for capturing frames from OpenCV VideoCapture.

    Runs in a separate thread, continuously reading frames and pushing them
    to a FrameQueue. Supports optional FPS limiting for video files.
    """

    def __init__(self, video_capture, target_fps: Optional[float] = None):
        """
        Initialize the capture worker.

        Args:
            video_capture: OpenCV VideoCapture object
            target_fps: Optional FPS limit (useful for video files)
        """
    ...
    def get_frame(self, timeout: float = 0.1) -> Optional[Tuple[np.ndarray, float, int]]:
        """
        ...
        """
        return self._frame_queue.get(timeout)
    ...
    def _capture_loop(self):
        ...
        frame_time = 1.0 / self._target_fps if self._target_fps else 0.0
